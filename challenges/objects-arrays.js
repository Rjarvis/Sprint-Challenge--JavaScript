...
graduates.forEach((value) => {
  var uniString = value.university;
  var pos = uniString.indexOf("Uni");
  if(pos > -1){
...
zooAnimals.forEach((value)=>{
  var Name = value.animal_name;
  var Scientific = value.scientific_name;
...
zooAnimals.forEach((value)=>{
  if(value.population < 5) lowerPopulation.push(value.animal_name);
});
...
var newTot= [];
zooAnimals.forEach((value)=>{
  newTot.push(value.population);
});
populationTotal = newTot.reduce((t,cV)=>{
  return t + cV;
});
